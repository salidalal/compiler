...
args_list:			no_args{ $$ = $1; }
				|	yes_args{ $$ = $1; }
				;
yes_args:			args ':' arg_type { $$ = makeNode("",$1,3,NULL); }							
				|	yes_args ';' args ':' arg_type {$$=makeNode("",$1,makeNode("",$1,3,NULL),NULL);}
				;

args:				id space {$$=makeNode("",$2,$1,NULL);}
				|	args ',' id space {$$=makeNode("",$1,$4,$3,NULL);}
				;
...
continue_proc:		'{' body '}' {$$=makeNode("BODY ",$2,NULL);}
				;

body:				/*empty body*/
				|	body statment {$$=makeNode("",$2,$1,NULL);}
				;
...
statment:				call semico {$$=makeNode("",$1,NULL);}
					|	if_statment 	{$$ = $1;}				
					|	for_statment  {$$=makeNode("FOR ",$1,NULL);}
					|	while_statment {$$=makeNode("WHILE ",$1,NULL );}
					|	initign_statment  {$$=makeNode("",$1,NULL);}
					|	the_ret semico	{ $$ = makeNode("",$1,NULL);}
					|	var_decls {  $$ = makeNode("",$1,NULL); }
					|	'{' statments '}'	{ $$=makeNode("",$1,NULL);}
					| 	/*epsilon*/ {}
					;
